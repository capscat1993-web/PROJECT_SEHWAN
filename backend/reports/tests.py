...
class ApiSmokeTests(TestCase):
    ...
    def test_company_dashboard_returns_payload(self):
        # (유)삼송창원공장 id=516 — periods 2019~2021, 주요재무지표 보유
        response = self.client.get("/api/companies/516/dashboard")
        self.assertEqual(response.status_code, 200)
        payload = response.json()
    ...
    def test_key_metrics_normalizes_recent_money_values_to_million_won(self):
        # (주)동남기계 id=526
        payload = _key_metrics_payload(526)
        self.assertEqual(payload["unit"], "백만원")
        self.assertAlmostEqual(payload["metrics"]["매출액"]["2024.12"], 59357.15, places=1)
        self.assertAlmostEqual(payload["metrics"]["영업이익"]["2024.12"], 3005.33, places=1)
        self.assertAlmostEqual(payload["metrics"]["당기순이익"]["2024.12"], 2834.14, places=1)

    def test_health_export_uses_template_and_populates_current_company(self):
        # (주)동남기계 id=526
        buffer, company_name = export_health_excel(526)
...
class FinancialHealthSmokeTests(TestCase):
    ...
    def test_health_service_marks_cashflow_missing_when_no_amount_data(self):
        ...
        # 평화홀딩스(주) id=507 — 현금흐름분석 섹션 없는 기업
        result = calculate_health(507)
